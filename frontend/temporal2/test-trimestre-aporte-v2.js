...
async function testTrimestreAporte() {
    ...
    try {
        ...
        // 5. Probar diferentes combinaciones válidas
        console.log('\n📋 5. Probando combinaciones válidas...');
        
        const validCombinations = [
            { trimestre: 1, aporte: 2, desc: 'Primer trimestre, Aporte 2' },
            { trimestre: 2, aporte: 1, desc: 'Segundo trimestre, Aporte 1' },
            { trimestre: 3, aporte: 2, desc: 'Tercer trimestre, Aporte 2' }
        ];
        ...
        for (const combo of validCombinations) {
            try {
                const testResponse = await axios.post(
                    ...
                    {
                        ...taskData,
                        nombre: `Tarea ${combo.desc}`,
                        trimestre: combo.trimestre,
                        aporte: combo.aporte
                    },
                    {
                        headers: {
                ...
                console.log(`✅ ${combo.desc}: Creada correctamente (ID: ${testResponse.data.task?.id})`);
            } catch (error) {
                console.log(`❌ Error creando tarea ${combo.desc}:`, error.response?.data?.message);
            }
        }
