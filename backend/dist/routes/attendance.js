...
router.get('/', async (req, res) => {
    try {
        const { course_id, parallel, start_date, end_date, student_id } = req.query;
        let where = {};
        if (course_id) {
            where.course_external_id = parseInt(course_id);
        }
        if (student_id) {
            where.student_external_id = parseInt(student_id);
        }
...
router.post('/', async (req, res) => {
    try {
        const { student_external_id, course_external_id, date, status, justification_file_reference } = req.body;
...
router.post('/bulk', async (req, res) => {
    try {
        const { course_external_id, date, records } = req.body;
        // Validaciones básicas
        ...
        for (const recordData of records) {
            const { student_external_id, status, justification_file_reference } = recordData;
...
router.put('/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { status, justification_file_reference } = req.body;
...
router.get('/stats', async (req, res) => {
    try {
        const { course_id, parallel, start_date, end_date } = req.query;
        let where = {};
        if (course_id) {
            where.course_external_id = parseInt(course_id);
        }
        ...
        const uniqueStudents = new Set(records.map((r) => r.student_external_id));
