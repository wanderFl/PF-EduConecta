...
const teachers_1 = require("../services/teachers");
...
router.get('/:teacherId', async (req, res) => {
    try {
        ...
        const teacherInfo = await (0, teachers_1.getTeacherInfo)(teacherId);
        if (!teacherInfo) {
...
router.get('/:teacherId/courses', async (req, res) => {
    try {
        ...
        const courses = await (0, teachers_1.getTeacherCourses)(teacherId);
...
router.get('/:teacherId/courses-with-subjects', async (req, res) => {
    try {
        ...
        const coursesWithSubjects = await (0, teachers_1.getTeacherCoursesWithSubjects)(teacherId);
        res.json({
            success: true,
...
router.get('/:teacherId/courses/:courseId/subjects', async (req, res) => {
    try {
        ...
        const hasAccess = await (0, teachers_1.verifyTeacherCourseAccess)(teacherId, courseId);
        if (!hasAccess) {
            return res.status(403).json({
                success: false,
        ...
        const subjects = await (0, teachers_1.getTeacherSubjectsByCourse)(teacherId, courseId);
        res.json({
            success: true,
            data: subjects,
...
router.get('/:teacherId/courses/:courseId/subjects/:subjectId/students', async (req, res) => {
    try {
        ...
        const hasAccess = await (0, teachers_1.verifyTeacherSubjectAccess)(teacherId, courseId, subjectId);
        if (!hasAccess) {
            return res.status(403).json({
                success: false,
        ...
        const students = await (0, teachers_1.getStudentsByCourseAndSubject)(teacherId, courseId, subjectId);
        res.json({
            success: true,
            data: students,
